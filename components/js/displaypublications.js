...
function renderPublications(publications) {
  ...
  paginatedItems.forEach((pub) => {
    ...
    post.innerHTML = `
          ...
          <div class="post-img me-4" style="flex: 0 0 220px;">
            <img src="${
              pub.imageURL || "../../assets/img/digital skills 2.webp"
            }" alt="${pub.title} Image"
          ...
          <div class="post-content" style="flex: 1 1 auto;">
            <p class="text-muted">
              ...
              <a href="${pub.id}" class="continue-reading" data-id="${
      pub.id
    }">Continue Reading</a>
            </p>
          </div>
  ...
  if (paginatedItems.length === 0) {
    container.innerHTML = "<p class='text-muted text-center'>No matching search results found!</p>";
    return;
  }  
  attachReadMoreHandlers();
...
async function populateSidebarInsights() {
  ...
  snapshot.docs.forEach((doc) => {
    ...
    if (
      ...
      (!latestUpdatedPost.dateModified ||
        data.dateModified?.toMillis() >
          latestUpdatedPost.dateModified?.toMillis())
    ) {
      latestUpdatedPost = {
        title: data.title || "Untitled",
  ...
  const averageContentLength = postsWithContent
    ? totalWords / postsWithContent
    : 0;
  ...
  document.getElementById("latestUpdatedPost").innerText =
    latestUpdatedPost.title;
  document.getElementById("averageContentLength").innerText = `${Math.round(
